...
#### 1. 移动端 UI/UX 布局 (单页 SPA)
- **视口规范**：完全适配 iPhone 灵动岛、刘海屏与底部 Home Indicator 安全区（`safe-area-inset`）。
- **底部 Tab 1【大盘/自选】**：宽基指数横向轮播卡片，自选股票卡片流，支持快速查看分时与一键跳转研判。
- **底部 Tab 2【智能推荐】**：AI 量化评分卡片流，展开核心逻辑与风险警示。
- **底部 Tab 3【个股研判】**：自选股下拉切换，触控手势双指缩放 K 线图（ECharts），大模型研报多段展示。
- **底部 Tab 4【虚拟操盘】**：总资产看板、持仓浮动盈亏卡片、快速一键平仓与模拟买入浮窗。

#### 2. 服务端与静态资源托管
- 模块路径：`app/web/`
  - `api.py`：定义 `/api/market/*`、`/api/watchlist/*`、`/api/stock/*`、`/api/recommend`、`/api/trading/*`；
  - `static/`：托管 `index.html`、`app.css`、`app.js`、`manifest.json` 与 `icon.png`。
- 启动引导：提供 `run_mobile_server.py`，启动时自动获取局域网 IP，并在终端与日志输出访问地址及 ASCII 二维码，手机微信/相机扫码即开。

---

*文档状态：正式设计基准，已包含版本管理与 iPhone 移动端 PWA 架构。*